...
Variable sar_trace_portals_record("sar_trace_portals_record", "1", "Record portal locations.\n");
...
void PlayerTrace::DrawPortalsAt(int tick) const {
	...
	for (const auto &[trace_idx, trace] : traces) {
		...
		auto atlas_prim   = Color( 32, 128, 210, 255);
		auto atlas_second = Color( 16,   0, 210, 255);
		auto pbody_prim   = Color(255, 180,  32, 255);
		auto pbody_second = Color( 58,   3,   3, 255);
...
PortalLocations PlayerTrace::ConstructPortalLocations() const {
	if (!sar_trace_portals_record.GetBool()) return PortalLocations{};
	console->Print("###########################################################################\n");
	console->Print("################## PlayerTrace::ConstructPortalLocations ##################\n");
	console->Print("###########################################################################\n");
	...
	for (int i = 0; i < Offsets::NUM_ENT_ENTRIES; ++i) {
		...
		if (strcmp(server->GetEntityClassName(ent), "prop_portal") == 0) {
			...
			if (portal.is_coop) {
				auto shooter = reinterpret_cast<void *>((uintptr_t)ent + Offsets::m_hFiredByPlayer);
				auto pplayer = server->GetPlayer(1);
				portal.is_atlas = shooter == pplayer;
				console->Print("shooter: %p, player: %p\n", shooter, pplayer);
			}
